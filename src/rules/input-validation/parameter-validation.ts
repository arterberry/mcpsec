...
export const parameterValidation: MCPSecurityRule = {
    ...
    async check(context: AnalysisContext): Promise<RuleViolation[]> {
        ...
        for (const tool of context.mcpServer.tools) {
            const toolViolations = await this.checkToolParameterValidation(tool, context);
            violations.push(...toolViolations);
        }

        // Check for validation schema consistency
        const schemaViolations = this.checkValidationSchemas(context);
  ...
  private async checkToolParameterValidation(tool: any, context: AnalysisContext): Promise<RuleViolation[]> {
        ...
        const schemaViolations = this.validateSchemaStructure(tool, context);
        violations.push(...schemaViolations);

        // Check implementation for validation logic
        const implViolations = await this.checkValidationImplementation(tool, context);
  ...
  private validateSchemaStructure(tool: any, context: AnalysisContext): RuleViolation[] {
        ...
        for (const [propName, propSchema] of Object.entries(schema.properties)) {
            const propViolations = this.validatePropertySchema(tool.name, propName, propSchema as any);
        ...
        if (context.config.foxCorp) {
            const foxViolations = this.validateFoxCorpSchema(tool, context);
  ...
  private validatePropertySchema(toolName: string, propName: string, propSchema: any): RuleViolation[] {
        ...
        if (propSchema.type === 'string') {
            const stringViolations = this.validateStringSchema(toolName, propName, propSchema);
            violations.push(...stringViolations);
        }

        // Check for number validation rules
        if (propSchema.type === 'number' || propSchema.type === 'integer') {
            const numberViolations = this.validateNumberSchema(toolName, propName, propSchema);
            violations.push(...numberViolations);
        }

        // Check for array validation rules
        if (propSchema.type === 'array') {
            const arrayViolations = this.validateArraySchema(toolName, propName, propSchema);
            violations.push(...arrayViolations);
        }

        // Check for object validation rules
        if (propSchema.type === 'object') {
            const objectViolations = this.validateObjectSchema(toolName, propName, propSchema);
  ...
  private validateStringSchema(toolName: string, propName: string, propSchema: any): RuleViolation[] {
        ...
        if (propSchema.enum) {
            for (const enumValue of propSchema.enum) {
                if (this.isDangerousEnumValue(enumValue)) {
                    violations.push({
                        ruleId: 'parameter-validation',
                        severity: 'error',
        ...
        if (this.isSensitiveField(propName) && !propSchema.pattern) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'error',
        ...
        if (this.needsFormatValidation(propName) && !propSchema.format) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'warning',
 ...
 private validateNumberSchema(toolName: string, propName: string, propSchema: any): RuleViolation[] {
        ...
        if (this.shouldBePositive(propName) && propSchema.minimum < 0) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'warning',
 ...
 private validateFoxCorpSchema(tool: any, context: AnalysisContext): RuleViolation[] {
        ...
        if (context.config.foxCorp?.streamingAssets && this.isStreamingTool(tool)) {
            const streamingViolations = this.validateStreamingParameters(tool, schema);
            violations.push(...streamingViolations);
        }

        // Conviva integration parameters
        if (context.config.foxCorp?.convivaIntegration && this.isConvivaTool(tool)) {
            const convivaViolations = this.validateConvivaParameters(tool, schema);
            violations.push(...convivaViolations);
        }

        // HAR validation parameters
        if (context.config.foxCorp?.harValidation && this.isHARTool(tool)) {
            const harViolations = this.validateHARParameters(tool, schema);
 ...
 private validateStreamingParameters(tool: any, schema: any): RuleViolation[] {
        ...
        if (schema.properties?.streamId && !this.hasStreamIdValidation(schema.properties.streamId)) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'error',
        ...
        if (schema.properties?.contentType && !this.hasContentTypeValidation(schema.properties.contentType)) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'error',
        ...
        if (schema.properties?.assetUrl && !this.hasAssetUrlValidation(schema.properties.assetUrl)) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'error',
 ...
 private validateConvivaParameters(tool: any, schema: any): RuleViolation[] {
        ...
        if (schema.properties?.sessionId && !this.hasSessionIdValidation(schema.properties.sessionId)) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'error',
        ...
        if (schema.properties?.metricName && !this.hasMetricNameValidation(schema.properties.metricName)) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'warning',
 ...
 private validateHARParameters(tool: any, schema: any): RuleViolation[] {
        ...
        if (schema.properties?.harData && !this.hasHARSizeValidation(schema.properties.harData)) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'error',
        ...
        if (schema.properties?.harContent && !this.hasHARFormatValidation(schema.properties.harContent)) {
            violations.push({
                ruleId: 'parameter-validation',
                severity: 'error',
 ...
 private async checkValidationImplementation(tool: any, context: AnalysisContext): Promise<RuleViolation[]> {
        ...
        const hasValidationLibrary = this.hasValidationLibraryUsage(implFile);
        ...
        const hasSchemaValidation = this.hasSchemaValidation(implFile);
        ...
        const hasValidationErrorHandling = this.hasValidationErrorHandling(implFile);
 ...
 private checkValidationSchemas(context: AnalysisContext): RuleViolation[] {
        ...
        const duplicateSchemas = this.findDuplicateSchemas(schemas);
