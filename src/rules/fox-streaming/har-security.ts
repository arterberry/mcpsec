...
export const harSecurity = {
    ...
    async check(context: AnalysisContext): Promise<RuleViolation[]> {
        ...
        for (const tool of harTools) {
            const toolViolations = await this.checkHARToolSecurity(tool, context);
            violations.push(...toolViolations);
        }

        // Check for HAR file handling security
        const fileViolations = this.checkHARFileHandling(context);
        violations.push(...fileViolations);

        // Check for sensitive data exposure in HAR files
        const dataViolations = this.checkHARDataSecurity(context);
    ...
    async checkHARToolSecurity(tool: any, context: AnalysisContext): Promise<RuleViolation[]> {
        const violations: RuleViolation[] = [];

        // Check for file size validation
        if (!this.hasFileSizeValidation(tool)) {
            violations.push({
                ruleId: 'har-security',
                severity: 'error',
        ...
        if (!this.hasContentValidation(tool)) {
            violations.push({
                ruleId: 'har-security',
                severity: 'error',
        ...
        if (!this.hasSensitiveDataFiltering(tool)) {
            violations.push({
                ruleId: 'har-security',
                severity: 'error',
        ...
        if (implFile) {
            const implViolations = await this.checkHARImplementation(implFile, tool);
    ...
    hasFileSizeValidation(tool: any): boolean {
    ...
    hasContentValidation(tool: any): boolean {
        if (!tool.inputSchema || !tool.inputSchema.properties) {
            return false;
        }
    ...
    hasSensitiveDataFiltering(tool: any): boolean {
    ...
    async checkHARImplementation(file: any, tool: any): Promise<RuleViolation[]> {
        const violations: RuleViolation[] = [];
        ...
        const visitor = (node: ts.Node) => {
            ...
            if (ts.isCallExpression(node)) {
                ...
                if (this.isUnsafeJSONParsing(callText)) {
                    violations.push({
                        ruleId: 'har-security',
                        severity: 'error',
                ...
                if (this.isSensitiveDataAccess(callText)) {
                    violations.push({
                        ruleId: 'har-security',
                        severity: 'warning',
            ...
            if (ts.isPropertyAccessExpression(node)) {
                ...
                if (this.isCredentialAccess(propAccess)) {
                    violations.push({
                        ruleId: 'har-security',
                        severity: 'error',
    ...
    isUnsafeJSONParsing(callText: string): boolean {
        return /JSON\.parse\(/i.test(callText) &&
            !/validate|check|limit|safe/i.test(callText);
    },

    isSensitiveDataAccess(callText: string): boolean {
    ...
    isCredentialAccess(propAccess: string): boolean {
    ...
    checkHARFileHandling(context: AnalysisContext): RuleViolation[] {
        const violations: RuleViolation[] = [];
        ...
        for (const file of harFiles) {
            ...
            if (this.hasSynchronousProcessing(file)) {
                violations.push({
                    ruleId: 'har-security',
                    severity: 'warning',
            ...
            if (!this.hasProperErrorHandling(file)) {
                violations.push({
                    ruleId: 'har-security',
                    severity: 'error',
    ...
    hasMemoryIssues(file: any): boolean {
    ...
    hasSynchronousProcessing(file: any): boolean {
        const content = file.content.toLowerCase();
        const syncPatterns = [
            /readfilesync/i,
            /json\.parse.*\)/,
    ...
    hasProperErrorHandling(file: any): boolean {
        const content = file.content.toLowerCase();
        const errorPatterns = [
            /try.*catch/,
            /catch.*error/,
    ...
    checkHARDataSecurity(context: AnalysisContext): RuleViolation[] {
        const violations: RuleViolation[] = [];
        ...
        for (const file of harFiles) {
            ...
            if (this.hasCredentialLogging(file)) {
                violations.push({
                    ruleId: 'har-security',
                    severity: 'error',
            ...
            if (this.hasInsecureDataStorage(file)) {
                violations.push({
                    ruleId: 'har-security',
                    severity: 'error',
    ...
    hasCredentialLogging(file: any): boolean {
        const content = file.content.toLowerCase();
    ...
    hasPIIExposure(file: any): boolean {
        const content = file.content.toLowerCase();
        const piiPatterns = [
            /email/i,
            /phone/i,
    ...
    hasInsecureDataStorage(file: any): boolean {
        const content = file.content.toLowerCase();
