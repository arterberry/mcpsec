...
const mockConfigManager = { getInstance: jest.fn() };
...
describe('CLI', () => {
	...
	beforeEach(() => {
		...
		mockConfigInstance = { /* mock config instance methods and properties */ };
		mockAnalyzerInstance = { /* mock analyzer instance methods and properties */ };
		mockReporterInstance = { /* mock reporter instance methods and properties */ };

		(mockConfigManager.getInstance as unknown as jest.Mock).mockReturnValue(mockConfigInstance);
		(mockAnalyzer as unknown as jest.Mock).mockReturnValue(mockAnalyzerInstance);
		(mockReporter as unknown as jest.Mock).mockReturnValue(mockReporterInstance);
	});
	...
	describe('End-to-End Security Analysis', () => {
		...
		beforeEach(() => {
			...
			(mockFs.readdirSync as jest.Mock).mockImplementation((dir: string, options?: any) => {
				const files = dir.includes('node_modules') ? [] : ['package.json', 'src'];

				if (options?.withFileTypes) {
					return files.map(name => ({
						name,
						isDirectory: () => name === 'src',
						isFile: () => name !== 'src',
						isBlockDevice: () => false,
          ...
          `;
			...
			it('should apply ignore patterns correctly', async () => {
				...
				(mockFs.readdirSync as jest.Mock).mockImplementation((dir: string, options?: any) => {
					...
					if (dir === tempDir) {
						files = ['src', 'test', 'legacy', 'package.json'];
					} else if (dir.includes('test')) {
						files = ['vulnerable.test.ts'];
					} else if (dir.includes('legacy')) {
						files = ['old-code.ts'];
					...
					if (options?.withFileTypes) {
						return files.map(name => ({
							name,
							isDirectory: () => ['src', 'test', 'legacy'].includes(name),
							isFile: () => !['src', 'test', 'legacy'].includes(name),
							isBlockDevice: () => false,
							isCharacterDevice: () => false,
							isSymbolicLink: () => false,
							isFIFO: () => false,
	...
	describe('Rule Combinations and Interactions', () => {
		...
		beforeEach(() => {
			...
			(mockFs.readdirSync as jest.Mock).mockImplementation((path: string, options?: any) => {
				if (options?.withFileTypes) {
					return [
						{
							name: 'package.json',
